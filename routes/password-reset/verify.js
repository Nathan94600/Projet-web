...
module.exports = (req, res, { passwordResetCodes }) => {
	if (req.method == "POST") {
		...
		req.on("data", chunk => body += chunk).on("end", () => {
			...
			else if (code != passwordResetCodes[email].code) errorMessage = "Le code de réinitialisation est incorrect";							

			if (errorMessage) res.writeHead(302, { location: `/mdp_oublie?email=${email}&errorMessage=${encodeURIComponent(errorMessage)}` }).end();
